...
export type TrackFilter = "all" | "precode100" | "blind75" | "dsa";
...
const BLIND_75_SET = new Set<string>(BLIND_75_IDS);

export function isProblemInTrack(
  problem: ProblemSummary,
  trackFilter: TrackFilter,
): boolean {
  if (trackFilter === "all") return true;
  if (trackFilter === "blind75") return BLIND_75_SET.has(problem.id);
  return (problem.track_id || "dsa") === trackFilter;
}

export function filterProblemsByTrack(
...
export function filterCategoriesByTrack(
  ...
  if (trackFilter === "blind75") {
    ...
    return derived.map((category) => ({
      ...category,
      section_description:
        metaById.get(category.id)?.section_description ??
