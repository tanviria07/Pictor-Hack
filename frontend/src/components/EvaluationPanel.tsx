...
interface EvaluationPanelProps {
  ...
  stepwise: StepwiseValidateResponse | null;
  stepwiseCode: string;
  inlineHint: InlineHintResponse | null;
  hintHistory: string[];
...
function statusPresentation(run: RunResponse): StatusPresentation {
...
function ResultHero({ run }: { run: RunResponse }) {
  ...
  return (
    <div
      className={`result-hero result-hero--${p.tone}`}
    ...
    >
      ...
      {total > 0 && (
        <div className="result-hero-metrics">
          <MetricPill
            label="Visible"
            passed={ev.passed_visible_tests}
            total={ev.total_visible_tests}
            tone={p.tone}
          />
          <MetricPill
            label="Hidden"
            passed={ev.passed_hidden_tests}
            total={ev.total_hidden_tests}
            tone={p.tone}
            hint="Inputs withheld"
          />
          <div
            className="result-hero-bar"
            role="progressbar"
            aria-valuemin={0}
            aria-valuemax={total}
            aria-valuenow={passed}
            aria-label={`${passed} of ${total} tests passing`}
          >
            <div
              className={`result-hero-bar-fill result-hero-bar-fill--${p.tone}`}
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
    ...
    total === 0
      ...
      : ratio === 1
        ...
        : ratio === 0
          ...
          : "warning";
  return (
    <div className={`metric-pill metric-pill--${pillTone}`}>
...
function VisibleTestGrid({ results }: { results: VisibleTestResult[] }) {
  if (!results.length) return null;
  return (
    <EvalSection
      title="Visible tests"
      hint={`${results.filter((r) => r.passed).length}/${results.length} passing`}
    >
      <div className="test-grid">
        {results.map((r) => (
          <div
            key={r.index}
            className={`test-chip test-chip--${r.passed ? "pass" : "fail"}`}
            title={r.label ?? `Test #${r.index + 1}`}
          >
...
function HiddenTestStrip({
  ...
  return (
    <EvalSection
      title="Hidden tests"
      hint={`${passed}/${total} passing — inputs withheld`}
    >
      <div className="test-strip">
        {cells.map((pass, i) => (
          <span
            key={i}
            className={`test-cell test-cell--${pass ? "pass" : "fail"}`}
            aria-hidden="true"
          />
        ))}
      </div>
...
function ErrorSection({
  ...
  return (
    <EvalSection
      title={isInternal ? "Platform" : "Execution error"}
      tone={isInternal ? "warning" : "error"}
    >
      <div
        className={`exec-error exec-error--${isInternal ? "internal" : "exec"}`}
      >
...
function InterviewerNotes({ text }: { text: string }) {
  if (!text?.trim()) return null;
  return (
    <EvalSection title="Interviewer notes">
      <blockquote className="notes-quote">{text}</blockquote>
    </EvalSection>
  );
}

function FocusAreas({ items }: { items: string[] }) {
  if (!items.length) return null;
  return (
    <EvalSection title="Focus areas">
      <ul className="focus-chips">
        {items.map((item, i) => (
          <li key={i} className="focus-chip">
            {item}
          </li>
        ))}
      </ul>
...
function HintHistory({
  ...
  return (
    <EvalSection
      title="Hint history"
      hint={hints.length > 0 ? `${hints.length} turn${hints.length === 1 ? "" : "s"}` : undefined}
    >
      {hints.length === 0 ? (
        ...
        <ol className="hint-list">
          {hints.map((hint, index) => (
            <li key={index} className="hint-card">
              <span className="hint-card-num">{index + 1}</span>
              <HintContent
                text={hint}
                onInsert={onInsertSnippet}
                className="hint-card-body"
              />
            </li>
          ))}
        </ol>
...
function EvalSection({
  ...
  return (
    <section className={`eval-section eval-section--${tone}`}>
...
function StepwisePanel({
  stepwise,
  stepwiseCode,
  onInsertSnippet,
}: {
  stepwise: StepwiseValidateResponse;
  stepwiseCode: string;
  onInsertSnippet: (snippet: string) => void;
}) {
  const isFail =
  ...
  const tone: StatusTone = stepwise.is_full_solution
    ? "success"
    : isFail
      ? "error"
      : "info";
  const bannerText = stepwise.is_full_solution
    ...
    : isFail
      ...
      : "Correct! Keep going.";
  const pct =
    stepwise.total > 0 ? (stepwise.correct_count / stepwise.total) * 100 : 0;
  ...
  return (
    ...
    >
      ...
      <div className="result-hero-metrics">
        ...
        >
          <div
            className={`result-hero-bar-fill result-hero-bar-fill--${tone}`}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
      ...
      {!stepwise.is_full_solution && stepwise.next_hint && (
        <div className="result-hero-extra">
          <HintContent
      ...
      {stepwise.is_full_solution && (
        <div className="result-hero-extra stepwise-success">
...
function IdleState({
  ...
  if (inlineHint) {
    return (
      <EvalSection title="Inline hint" tone="info">
...
export function EvaluationPanel({
  ...
  const caseNote = useMemo(
    () =>
      run?.evaluation.failing_case_summary?.trim()
      ...
      <div className="eval-scroll">
        ...
        {run && !stepwise && (
          <>
            ...
            <HiddenTestStrip
              passed={run.evaluation.passed_hidden_tests}
              total={run.evaluation.total_hidden_tests}
            />
            <ErrorSection
              status={run.status}
              errorType={run.evaluation.error_type}
              errorMessage={run.evaluation.error_message}
            />
            {caseNote && (
              <EvalSection title="Case note">
...
function HourglassIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path
        d="M7 3h10M7 21h10M7 3v3.5a5 5 0 0010 0V3M7 21v-3.5a5 5 0 0110 0V21"
        stroke="currentColor"
        strokeWidth="2.2"
        strokeLinecap="round"
        strokeLinejoin="round"
